...
Waarom dit bestaat: 1253 servermodules en 2743 endpoints houdt niemand in zijn hoofd.
...
|---|---|
| API-endpoints | 2743 |
| servermodules (`server/**/*.js`) | 1437 |
| routebestanden (`server/routes/**`) | 337 |
| kernmodules (`server/kern/**`) | 805 |
| schermen (`public/**/*.html`) | 209 |
| gedeelde browsermodules (`public/shared/*.js`) | 163 |
| toetsbestanden (`test/*.test.js`) | 589 |
...
Daarnaast 1396 `/api/`-endpoints buiten deze acht: de infra (health, stream, push,
...
| Meting | Nu |
|---|---|
| kern-namen die routes aanraken | 1071 |
| daarvan door **meer dan één** domein (de echte koppeling) | 140 |
| daarvan door precies één domein | 931 |
...
```
app(123) auth(70) supplierAuth(46) db(30) officeAuth(28) accounts(18) liveCodename(17)
...
keyVanCodenaam(7) payrollOS(7) boardroomWie(6) geenGast(6) noteFailedTry(6) kern(6)
