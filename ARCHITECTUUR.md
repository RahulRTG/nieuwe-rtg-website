...
Waarom dit bestaat: 1253 servermodules en 4894 endpoints houdt niemand in zijn hoofd.
...
|---|---|
| API-endpoints | 4894 |
| servermodules (`server/**/*.js`) | 3051 |
| routebestanden (`server/routes/**`) | 554 |
| kernmodules (`server/kern/**`) | 1934 |
| schermen (`public/**/*.html`) | 287 |
| gedeelde browsermodules (`public/shared/*.js`) | 310 |
| toetsbestanden (`test/*.test.js`) | 1347 |
| schermtoetsen (`test/*.e2e.js`) | 196 |
...
zie §5 -- er zijn nog 225 kern-namen die meer dan één domein aanraakt.
...
Daarnaast 3299 `/api/`-endpoints buiten deze acht: de infra (health, stream, push,
...
| Meting | Nu |
|---|---|
| kern-namen die routes aanraken | 1529 |
| daarvan door **meer dan één** domein (de echte koppeling) | 225 |
| daarvan door precies één domein | 1304 |
...
```
app(193) auth(117) supplierAuth(64) officeAuth(43) db(37) liveCodename(35) status(28)
