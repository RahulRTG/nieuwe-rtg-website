...
Getoetst in `test/rtfos.test.js` (twaalf scenario's op de governance-grendels), `test/rtfos-uitvoering.test.js` (zes op de uitvoering, zeven mutaties), `test/rtfos-portalen.test.js` (acht op de portalen en dossiers) en `test/rtfosschermen.e2e.js` (de twee schermen, inclusief het rauwe antwoord aan de gemeente). De 30%-afdracht van RTG naar de stichting blijft waar hij stond (`kern/fonds.js`); dit OS gaat over wat de stichting met dat geld dóét.
