...
De RTFoundation is niet één kantoor dat alles zelf doet, maar een landelijke stichting die per stad met bestaande lokale partijen samenwerkt. Het **Foundation OS** (`server/kern/rtfos/`, routes `/api/rtfos/*`, schermen `/apps/foundation/os.html`, `os-bestuur.html`, `os-portaal.html`, `os-vrijwilliger.html`, `os-deelnemer.html` en `os-publiek.html`) draagt dat federatieve model: **eigen uitvoering per stad, centrale governance erboven**. Wat het landelijke toezicht draagt staat in code en niet in een afspraak.
