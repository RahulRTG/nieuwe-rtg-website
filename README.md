...
- **Een aanspraak hangt aan een grond en nooit aan een boolean** — een **herkomst** (aankoop, cadeau, werkgever, kaartje, abonnement, promotie, organisator, maker) plus een **bron** (de gebeurtenis zelf). Zonder allebei ontstaat er geen aanspraak. Verlenen doet de maker, en alleen voor een code die een van zijn eigen partituren werkelijk vraagt; aan uzelf verlenen kan niet. **Dezelfde bron verleent maar één keer**: een herhaald verzoek is een netwerkherhaling en geen tweede aankoop, en levert dus dezelfde aanspraak op in plaats van een tweede. Een échte tweede aankoop draagt een andere bron. Dat onderscheid staat er nu al, vóór er geld aan hangt — daarna is het de plek waar een dubbele verwerking wel of niet ontstaat. Het is een **huiswoord, geen schermwoord**: een lid ziet "jouw aankopen", nooit "mijn aanspraken".
...
| `POST /api/uitvoering/bon` `{partituurId}` | wat u betaalt, aan wie, wat u krijgt — en wat RTG hier **niet** doet |
| `POST /api/uitvoering/koop` `{partituurId, idem}` | de aankoop; de boeking wordt de grond onder de aanspraak |
...
**De keten is rond: aanbod → aankoop → aanspraak → uitvoering.** Een maker zet een prijs op een partituur (`kern/uitvoering/aanbod.js`); een lid ziet eerst de **bon** en bevestigt dan zelf de **koop** — twee stappen, want alles wat een ander raakt is maximaal klaarzetten (`GELD.md` par. 3). De betaling loopt over dezelfde rail als de verkoopzone van het Podium (`pay.stuur`, lid aan maker); er komt geen tweede geldstroom bij en dit bestand boekt niets zelf.

**De hele keten is idempotent, en dat is het scharnier.** `pay.stuur` is idempotent op zijn `idem` en geeft bij een herhaling **dezelfde boeking** terug; die boeking is de `bron` van de aanspraak, en een aanspraak wordt per `(code, bron)` maar één keer verleend. Een dubbeltik levert dus één betaling en één aanspraak op — niet twee van allebei, en vooral niet één van het één en twee van het ander. Er ligt een tweede vangnet bovenop (wie het al heeft, komt niet bij de betaling), en beide zijn apart getoetst.

**Twee grenzen bij de prijs.** Een prijs zonder benodigde aanspraak bestaat niet — dan zou iemand betalen voor een deur die toch al openstaat. En de aanspraak losweken terwijl er een prijs op staat wordt geweigerd, want dat zou een betaald werk voor iedereen openzetten.

**Wat er níét is, en dat staat er met de reden bij.** RTG maakt van zo'n aankoop **geen btw-factuur** op en verzorgt geen retourregeling: dit is dezelfde rail en dus dezelfde grens als bij de verkoopzone van het Podium, waar dat als open punt staat (`TAKEN.md` 4.16). De bon zegt dat met zoveel woorden in plaats van een tarief te verzinnen. De vierde kolom van een partituur — de **handelingen** vanuit een lopende uitvoering — is niet gebouwd, dus er staat geen knop die doet alsof. En er is nu ook een **scherm**: `/apps/uitvoering.html`, met twee standen -- de kijker kiest een partituur en zegt hoeveel tijd hij heeft, de maker stelt er een samen uit zijn eigen werk. Twee dingen zijn daar met opzet anders dan in een speler. Het **bewijs staat even groot als de montage zelf** (waaruit dit bestaat, wat er niet in zit en waarom, wat de maker toestaat) en niet in de kleine lettertjes. En een **weigering rendert als antwoord**: wie om 30 seconden vraagt van een werk waarvan de kern er 60 duurt, leest hoe lang die kern werkelijk is -- geen rood kruis. `test/uitvoering.e2e.js` legt allebei vast, en zakt als het bewijsblok verdwijnt of als een weigering een foutmelding wordt. Zet de boardroom de schakelaar `uitvoering` uit, dan blijft de mediawereld eronder gewoon werken.

`test/uitvoering.test.js` legt tweeëntwintig dingen vast, waaronder de vijf die het makkelijkst stilletjes zouden verdwijnen: een budget onder de kern **weigert** (en levert geen korter werk), een partituur gaat alleen over **eigen** werk, zonder aanspraak komt er geen uitvoering én geen inhoudsopgave, een verdwenen kern weigert, en dezelfde bron verleent maar één aanspraak. Alle vijf zijn met een mutatie **zien zakken** (LAT.md regel 2), net als de drie van de koopketen — waaronder de scherpste: vervang het boeking-id als `bron` door een verzonnen sleutel en de keten valt uit elkaar; de mutatiemotor zet het bestand op *scherp: gezakt* — het zakt op de inhoud en niet alleen op de inlog.

**Gemeten, niet aangenomen.** Eén uitvoering van twee fragmenten kost p50 3,2 ms / p95 4,3 ms bij een kleine bibliotheek en p50 3,4 ms / p95 7,0 ms bij 105 wereldrijen. De motor leest de catalogus van de Media OS in zijn geheel — dat ziet er duur uit, maar de vier domeinen begrenzen zichzelf al (Clips levert een eindige dagselectie), dus de kosten lopen niet weg met de bibliotheek. Metrics, de rem op de deur en de SLO-doelen voor beschikbaarheid en p90 gelden hier automatisch: ze hangen aan het routepatroon en aan `route: "*"`, niet aan een lijst die per domein bijgewerkt moet worden.
