    ...
    const html =
      ...
      '<div style="padding:1rem 1.1rem 1.4rem;">' +
        '<div style="display:flex;align-items:center;gap:0.6rem;"><b style="font-size:1.1rem;font-family:\'Bodoni Moda\',serif;">' + escT(p.name) + '</b>' +
          '<button id="etaVolg" style="margin-left:auto;background:' + (p.volgIk ? 'var(--rtg-leesgoud,var(--goud))' : 'none') + ';color:' + (p.volgIk ? '#000' : 'var(--rtg-leesgoud,var(--goud))') + ';border:1px solid var(--rtg-leesgoud,var(--goud));border-radius:999px;padding:0.3rem 0.9rem;font-size:0.72rem;font-weight:600;font-family:inherit;cursor:pointer;">' + (p.volgIk ? '✓ ' + T('sal.volgt','Volgt') : '+ ' + T('sal.volg','Volg')) + '</button></div>' +
        ...
        (vz.length ? '<div class="h-mt80">' + vz.map(v =>
          '<div style="border:1px solid var(--rtg-leesgoud,var(--goud));border-radius:12px;padding:0.7rem 0.9rem;margin-top:0.5rem;background:var(--card);">' +
        ...
        (items.length
          ? items.map(it =>
            ...
            (it.folder ? '<div style="font-weight:600;margin-top:0.2rem;">' + escT(it.folder.titel) + '</div>' +
              ...
              ((it.folder.items && it.folder.items.length) ? '<div style="margin-top:0.45rem;display:grid;gap:0.2rem;">' + it.folder.items.map(x => '<div style="display:flex;justify-content:space-between;font-size:0.8rem;"><span>' + escT(x.naam) + '</span>' + (x.prijs != null ? '<span style="color:var(--rtg-leesgoud,var(--goud));">' + eur2(x.prijs) + '</span>' : '') + '</div>').join('') + '</div>' : '')
              : (it.deal ? '<div style="font-weight:600;margin-top:0.2rem;">' + escT(it.deal.titel) + (it.deal.mijnCode ? ' · <span style="color:var(--rtg-leesgoud,var(--goud));">' + it.deal.mijnCode + '</span>' : '') + '</div>'
