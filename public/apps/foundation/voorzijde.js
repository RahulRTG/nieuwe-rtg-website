...
(function(w,d){
  'use strict';
  var gestart=false,info=null,toonScherm=null,agendaItems=[],agendaState='loading';
  ...
  function agenda(){var van=new Date(),tot=new Date();tot.setDate(tot.getDate()+7);return w.Sessie.api('/gezin/agenda/bereik',{code:w.Sessie.huidig().code,token:w.Sessie.huidig().token,van:iso(van),tot:iso(tot)}).then(function(x){agendaItems=x.items||[];agendaState='ready';w.RTGFoundationVoorzijdeBeeld.dag(info,agendaItems,agendaState)},function(){agendaState='error';w.RTGFoundationVoorzijdeBeeld.dag(info,[],'error')})}
  ...
  function start(data,toon){info=data||{};toonScherm=toon;d.documentElement.setAttribute('data-rtf-foundation-voor','1');bind();agendaState='loading';agendaItems=[];w.RTGFoundationVoorzijdeBeeld.dag(info,[],'loading');w.RTGFoundationVoorzijdeBeeld.groei(info,null,null);w.RTGFoundationVoorzijdeBeeld.kring(info,null);agenda();if(w.RTGGlyf&&w.RTGGlyf.vul)w.RTGGlyf.vul()}
  w.addEventListener('rtglang',function(){if(info)w.RTGFoundationVoorzijdeBeeld.dag(info,agendaItems,agendaState)});
