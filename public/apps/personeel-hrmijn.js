...
(function () {
    ...
    (d.inwerk || []).filter(function (t2) { return !t2.klaarOp; }).forEach(function (t2) {
      ...
      h += '<div style="' + kaartStijl + '"><div style="' + kopStijl + '">Mijn inwerklijst · ' + n + '/' + t2.stappen.length + '</div>' +
        ...
        t2.stappen.map(function (s) {
          return '<button data-pdhrvink="' + t2.id + '" data-stap="' + s.id + '" style="display:block;width:100%;text-align:left;margin-top:0.5rem;padding:0.5rem 0.7rem;border-radius:10px;border:1px solid var(--line,rgba(255,255,255,0.1));background:none;color:inherit;font:inherit;font-size:0.78rem;cursor:pointer;' + (s.klaar ? 'opacity:0.65;' : '') + '">' + (s.klaar ? '✓ ' : '○ ') + esc(s.tekst) + '</button>';
        }).join('') + '</div>';
    });
  ...
  function tekenLoon(stroken, kans) {
    ...
    h += '<button id="pdLoonOpen" style="display:block;width:100%;text-align:left;margin-top:0.5rem;padding:0.5rem 0.7rem;border-radius:10px;border:1px solid var(--line,rgba(255,255,255,0.1));background:none;color:inherit;font:inherit;font-size:0.78rem;cursor:pointer;">' +
