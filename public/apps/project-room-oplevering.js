(function (w) {
  ...
  function teken(root, staat, p) {
    ...
    if (p.status === 'afgerond' && p.oplevering) {
      vak.innerHTML = '<article class="pr-resultaat"><span class="pr-label klaar">Menselijk opgeleverd</span><h2>' + U.veilig(p.oplevering.uitkomst) + '</h2><p>' + U.veilig(p.oplevering.leren || 'Er is geen afzonderlijke leerregel vastgelegd.') + '</p><div class="pr-voortgang pr-voortgang--vol"><i></i></div><div class="pr-metertekst"><span>100% afgerond</span><span>' + U.veilig(p.oplevering.door || 'Projecteigenaar') + '</span></div></article><div class="pr-bewijskop"><h2>Bewijs van uitvoering</h2><span class="pr-label">' + bewijs + ' vastgelegd</span></div>' + bewijsRijen(p); return;
    }
