...
(function(w,d){
  ...
  w.RTGCommandWerktafel=function(o){
    ...
    /* Aan het eind hiervan stonden twee open()-aanroepen: Reizen & Veilig en
       Geld gingen vanzelf open. De werktafel is wel weer het beginscherm, maar
       LEEG -- welke twee apps dat zouden moeten zijn is een keuze van een mens
       en niet van het huis. */
    ...
    function bouwNav(){var nav=root.querySelector('.cmd-nav');
      var k=d.createElement('button');k.className='cmd-klok';k.innerHTML=svg('home')+'<span>Beginscherm</span>';
      k.onclick=function(){dichtLade();o.thuis()};nav.appendChild(k);
      APPS.forEach(function(a){var b=d.createElement('button');b.innerHTML=svg(a[2])+'<span>'+a[0]+'</span>';b.dataset.url=a[1];b.onclick=function(){dichtLade();o.open(a[1],a[0]);};nav.appendChild(b)})}
