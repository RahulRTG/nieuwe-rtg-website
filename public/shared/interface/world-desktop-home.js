  ...
  function start() {
    ...
    w.fetch('/shared/interface/world-widget-catalog.json', { credentials: 'same-origin' }).then(function (r) {
      ...
      var frame = w.RTGDesktopFrameHost({ root: root, home: home, favorites: favorites, surface: surface,
        paths: new Set(all.map(function (a) { return a.url.split(/[?#]/)[0]; })),
      ...
      var runtime = w.RTGWorkspaceRuntime({ workspaceId: 'desktop-' + world,
        services: { familyRequest: function (path, body) {
          ...
          if (!['/api/rtf/leerling/dag', '/api/rtf/leren/schrijfsels'].includes(path)) return Promise.reject(new Error('unknown-family-source'));
      ...
      greet();
      if (w.RTGIdentityRuntime().authenticated()) runtime.execute('desktop.widget.read', { id: 'verificatie' }).then(function (j) {
