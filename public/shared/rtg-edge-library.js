...
(function (w) {
  ...
  function casco(cfg, s) {
    return '<header class="rtg-edge-top"><a class="rtg-edge-mark" href="' + cfg.home + '" aria-label="Rahul Travel Group, naar ' + cfg.naam + '"><span class="rtg-edge-mark-short" translate="no" aria-hidden="true">RTG</span><span class="rtg-edge-mark-lockup" aria-hidden="true"><strong translate="no">Rahul Travel Group</strong><small>Experience the elite class</small></span></a><nav class="rtg-edge-crumbs" aria-label="U bent hier"></nav><nav class="rtg-edge-worldbar" aria-label="De vier RTG-werelden"></nav><button class="rtg-edge-state" type="button" aria-label="Beveiliging en systeemstatus" aria-expanded="false"><i></i><span>Controleren</span></button></header>' +
      ...
      '<footer class="rtg-edge-bottom"><button class="rtg-edge-menu" type="button" aria-label="Menu en alle functies" aria-expanded="false">' + s('menu') + '<small>MENU</small></button><a href="' + cfg.home + '" aria-label="Naar home">' + s('home') + '<small>HOME</small></a><button class="rtg-edge-worlds-trigger" type="button" aria-label="Werelden openen">' + s('grid') + '<small>WERELDEN</small></button><span class="rtg-edge-history"><button type="button" data-go="back" aria-label="Terug">' + s('back') + '</button><button type="button" data-go="next" aria-label="Vooruit">' + s('next') + '</button></span><button class="rtg-edge-layout" type="button" aria-label="Aantal schermen">' + s('grid') + '<small>1</small></button><div class="rtg-edge-action"><button type="button" data-rtg-edge-primary></button></div><button class="rtg-edge-actions-trigger" type="button" aria-label="Acties van dit scherm">' + s('list') + '<small>ACTIES</small></button><div class="rtg-edge-appslot" aria-label="Functies van dit scherm"></div><button class="rtg-edge-ai" type="button" aria-label="Gesprek met Rahul" aria-expanded="false"><span class="rtg-edge-mouth"></span></button></footer>';
  }
