...
(function () {
  ...
  function bouw() {
    // Op het leden-OS (app.html) en binnen een OS-venster (iframe) verhuizen de
    // knoppen "Beeld draaien" en "Volledig scherm" naar het bedieningspaneel;
    // dan tekenen we hier geen zwevende pil op het scherm. window.RTGscherm
    // blijft bestaan, zodat het paneel de functies gewoon kan aanroepen; een
    // eerder gekozen hoek passen we wel meteen toe.
    var opOS = /\/apps\/app\.html$/.test(location.pathname);
    var inFrame = false; try { inFrame = window.self !== window.top; } catch (e) { inFrame = true; }
    if (opOS || inFrame) { if (hoek) pasHoekToe(); return; }
    stijl();
