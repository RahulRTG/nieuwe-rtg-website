...
function dekking() {
  ...
  /* WAAROM HIER EEN INDEX STAAT EN GEEN LUS MET includes().

     Dit was zeven keer `testTekst.includes(...)` per route, en testTekst is de
     samengeplakte inhoud van alle toetsbestanden -- tientallen megabytes. Bij
     ruim vierduizend routes zijn dat bijna dertigduizend volledige scans over
     die tekst. Gemeten: 64% van de HELE keuring zat in deze ene functie, 21 van
     de 33 seconden. En de keuring is geen eenmalig ding -- scripts/norm.js
     roept hem aan, test/meterijk.test.js roept norm aan voor ELKE meter, en die
     toets liep daardoor tegen twaalf minuten aan.
     ...
     doorlopen en er een register van maken.
       ...
       - `naApi` is alles wat er in de toetsen direct achter een '/api/' staat.
         ...
         met de staart van deze route. Gesorteerd, met een tweedeling gezocht.
       - `letterlijk` is de verzameling teksten die in de toetsen TUSSEN twee
         aanhalingstekens staan. `includes("'" + vorm + "'")` is dan precies:
         zit `vorm` in die verzameling.

     Allebei zijn ze exact hetzelfde antwoord en geen benadering; split() op een
     scheidingsteken geeft per definitie de stukken die er tussen staan. Dat is
     ook nagemeten: de keuring geeft na deze wijziging een byte-voor-byte
     identiek --json-rapport, en test/keuringsnelheid.test.js houdt die
     gelijkheid vast met de trage vorm als tegenspeler. */
  const gedekt = maakDekkingsIndex(testTekst);
