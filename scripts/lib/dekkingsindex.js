...
function maakDekkingsIndex(testTekst) {
  ...
  return function gedekt(route) {
    /* Een pad zonder /api-prefix komt van scripts/deltapoort.js. Daar telt het
       aantal routes op de vingers van een hand, dus die tak mag de volle
       zoektocht doen -- precies zoals de trage vorm hem deed, zodat het
       antwoord hetzelfde blijft. */
    if (!route.startsWith('/api/')) {
      if (tekst.includes(route)) return true;
      const kaal = route.replace(/^\//, '');
      return letterlijk.has(kaal) || letterlijk.has('/' + kaal);
    }
