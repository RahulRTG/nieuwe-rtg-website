/* WAT IS ER MET REDEN OPENBAAR? -- de ene lijst.

   Elke regel hier is een BEWUSTE keuze en geen omissie. Wie er een toevoegt,
   schrijft er een reden bij die klopt; kun je dat niet, dan is de route
   waarschijnlijk gewoon een gat.

   WAAROM HIJ HIER STAAT EN NIET IN check.js. Hij zat als lokale variabele in
   keuringsregel 28, waar niemand anders erbij kon. Toen scripts/handlerwacht.js
   de 612 routes zonder bewakerslaag ging beoordelen, had die dezelfde vraag te
   beantwoorden -- welke daarvan zijn met opzet open -- en de keuze was: de lijst
   overtypen of hem delen. Overtypen is precies de dubbeling waar LAT.md regel 4
   over gaat, en juist op deze lijst: twee lijsten van wat openbaar mag zijn
   lopen uiteen, en de losse van de twee wordt de ruimere.

   De inhoud is woordelijk verhuisd; er is geen regel bijgekomen of afgegaan.
   Wie een pad toevoegt, doet dat hier, en zowel de keuring als de handlerwacht
   zien het meteen. */
'use strict';
...
const PUBLIEK = new Map([
  ...
  ['/api/lab2/publiek/apparatuur', 'de leenbare apparatuur van een lab, met de eisen erbij; wie iets wil lenen heeft geen RTG-account en dat hoort geen drempel te zijn'],
  ['/api/lab2/publiek/uitleen-aanvraag', 'idem: een school vraagt een apparaat aan. Het lab beslist erover met een naam eronder; de aanvraag zelf is geen bevoegdheid'],
  ['/api/lab2/publiek/vragen', 'de buurtvragen met hun stand -- juist ook de afgewezen, met de reden; wie zijn vraag niet kan terugvinden, heeft geen antwoord gekregen'],
