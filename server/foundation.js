...
/* ---------- in de app berichten en (beeld)bellen tussen gezinsleden ----------
   Zo hoeft niemand een telefoonnummer van moeder of de kinderen te delen: je
   bereikt elkaar via de app. Alleen de noodnummers staan bij de belangrijke info.
   Chatberichten liggen versleuteld; een live SSE-kanaal per gezin bezorgt chat
   en de WebRTC-belsignalen realtime. */
...
function meldNaarGast(profiel, g, van, tekst, soort) {
  ...
  if (pushHook) { try { pushHook(profiel.koppel.userId, { title: 'RTFoundation · ' + g.naam, body: van + ': ' + String(tekst).slice(0, 120), tag: 'rtf-chat-' + van }); } catch (e) {} }
}
...
router.post('/gezin/bel', (req, res) => {
  ...
  if (kind === 'ring' && naarP.koppel && naarP.koppel.userId && pushHook) {
    try { pushHook(naarP.koppel.userId, { title: 'RTFoundation · ' + s.g.naam, body: s.p.naam + ' belt je' + (req.body.video ? ' (video)' : ''), tag: 'rtf-bel' }); } catch (e) {}
  }
  res.json({ ok: true });
});
...
/* ---------- Marktplaats: gezinnen kopen en verkopen (gedeelde motor
