...
module.exports = (ctx) => {
  ...
  /* KIJKEN ZONDER SCHEPPEN, en dat onderscheid is hier geen netheid.
     ...
     `bak()` zet de la neer voor een gezin dat er nog geen had. Op de weg die
     werkelijk iets TOEVOEGT hoort dat ook zo. Op elke andere weg is het
     verkeerd: wie een wens opzoekt die niet bestaat krijgt een 404, en dan
     hoort er niets achter te blijven -- de statuscode en de database zeggen
     anders twee verschillende dingen over hetzelfde verzoek.

     Bestaat de la wel, dan geeft dit de ECHTE lijsten terug en geen kopie: wat
     je hier vindt en aanpast (een wens claimen) landt gewoon in de opslag.
     Bestaat hij niet, dan krijg je een lege vorm die nergens aan vastzit, en
     dan loopt elke zoektocht dood op een nette 404. */
  const kijk = (g) => {
    const v = (g && g.verjaardagen && typeof g.verjaardagen === 'object') ? g.verjaardagen : {};
    return {
      mensen: Array.isArray(v.mensen) ? v.mensen : [],
      wensen: Array.isArray(v.wensen) ? v.wensen : [],
      potjes: (v.potjes && typeof v.potjes === 'object') ? v.potjes : {}
    };
  };
  ...
  router.post('/gezin/verjaardag/persoon', (req, res) => {
    ...
    bak(s.g).mensen.push(m); save();
    res.json({ ok: true, persoon: m });
  });
  router.post('/gezin/verjaardag/persoon/verwijder', (req, res) => {
    const s = familieVan(req, res); if (!s) return;
    const m = kijk(s.g).mensen.find(x => x.id === req.body.persoonId);
  ...
  router.post('/gezin/verjaardag/potje/bijdrage', (req, res) => {
    const s = familieVan(req, res); if (!s) return;
    const persoon = kijk(s.g).mensen.find(x => x.id === req.body.persoonId);
    ...
    /* De vol-controle gaat VOOR het aanmaken van het potje: anders staat er na een
       geweigerd verzoek een leeg potje dat er niet was. Een bestaand potje wordt
       hier gewoon gelezen; een niet-bestaand potje heeft nul bijdragen en haalt
       de grens dus nooit. */
    const bestaand = kijk(s.g).potjes[persoon.id];
    if (bestaand && (bestaand.bijdragen || []).length >= 200) return res.status(400).json({ error: 'Het potje heeft al veel bijdragen.' });
    const v = bak(s.g);
    if (!v.potjes[persoon.id]) v.potjes[persoon.id] = { doel: 0, bijdragen: [] };
    v.potjes[persoon.id].bijdragen.push({ id: rid(3), pid: s.p.id, bedrag, at: nu() }); save();
    res.json({ ok: true });
  });
