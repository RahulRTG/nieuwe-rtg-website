/* Functieschakelaars, deel "duiden" (server/functies/voorstel): de lokale
   taal-hulp. Begrijp een korte Nederlandse instructie en stel wijzigingen voor.
   Dit is de terugval als er geen AI-sleutel is, en houdt de controlekamer ook
   zonder externe AI bruikbaar. Herkent intentie (aan/uit), doelgroep(en) en
   welke functie(s) of categorie het betreft, plus de geld-regie (pasprijs,
   korting, commissie). Verbatim afgesplitst uit voorstel.js; valideerVoorstel
   komt binnen via de orkestrator (index.js) om een kringverwijzing te vermijden. */
const { CATEGORIEEN, DOELGROEPEN, DOELGROEP_OP_ID, LEDEN, FUNCTIES } = require('../register');
const { functieAanVoor } = require('../toegang');

module.exports = (valideerVoorstel) => {
