...
module.exports = function maakInlog({ swStart, swZeg, ord, schoon, gesprekken }) {
  ...
  /* Elke stap van het inlogpad draagt `entree: true` mee: de poort toont dan
     de kopregel "De entree" -- de spiegel van de ballotage voor wie al lid
     is. De stap met `login` erbij is het wachtwoordmoment; daar zet de poort
     ook de kluisregel bij. */
  function naarWoordInlog(g, u) {
    g.login = { u };
    g.stap = 'login-af';
    const swKan = typeof swStart === 'function';
    return { entree: true, tekst: 'Welkom terug. Typ je wachtwoord hieronder; het gaat rechtstreeks de kluis in, niet door dit gesprek.' +
  ...
  function naarSleutelwoorden(g) {
    ...
    if (r && r.error) { g.stap = 'login-naam'; return { entree: true, tekst: r.error } }
    g.sw = { id: r.id };
    g.stap = 'sw-open';
    return { entree: true, tekst: 'Goed, we doen het met je sleutelwoorden: verweef je ' + ord(r.posA) + ' en je ' + ord(r.posB) + ' sleutelwoord losjes in een zin. (Toch het wachtwoord? Zeg "wachtwoord".)' };
  }
  ...
  async function inlogStap(g, tekst, id) {
    switch (g.stap) {
      case 'login-naam': {
        ...
        if (!u || u.length < 2) return { entree: true, tekst: 'Welk e-mailadres of welke gebruikersnaam gebruik je hier? Typ hem even voluit.' };
        return naarWoordInlog(g, u);
      }
      case 'sw-open': {
        if (/\bwachtwoord\b/i.test(tekst)) { g.stap = 'login-af'; g.sw = null; return { entree: true, tekst: 'Ook goed. Typ je wachtwoord hieronder; het gaat rechtstreeks de kluis in, niet door dit gesprek.', login: g.login || null }; }
        ...
        return { entree: true, tekst: 'Dank je.' + echo + ' Sluit nu af met je ' + ord(r.posSluit) + ' sleutelwoord.' };
      }
      case 'sw-sluit': {
      ...
      case 'login-af': {
        ...
        return { entree: true, tekst: 'Typ je wachtwoord gewoon hieronder, dan ben je zo binnen. Kom je er niet uit: zeg "opnieuw" of "wachtwoord vergeten".', login: g.login || null };
      }
      case 'vergeten-mail': {
        const mail = /[^@\s]+@[^@\s]+\.[^@\s]+/.exec(tekst);
