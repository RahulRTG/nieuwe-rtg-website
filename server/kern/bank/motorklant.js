...
module.exports = function maakBankMotorklant() {
  ...
  return {
    ...
    async bankSaldiSnapshot() {
      if (!aan) return { error: 'Rust-motor staat uit; geen native banksaldi opgevraagd.', status: 503 };
      const af = new AbortController();
      const t = setTimeout(() => af.abort(), TIMEOUT_MS);
      try {
        const r = await fetch(URL + '/api/bank/saldi', { method: 'POST', headers: koppen(), body: '{}', signal: af.signal });
