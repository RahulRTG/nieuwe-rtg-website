...
module.exports = (ctx) => {
  const { opdrachten, boekAsync, rekMeta, seintje } = ctx;

  opdrachten.registreerTeruggang('sepa-uit', async (o) => {
    const terug = await boekAsync({ van: 'extern:sepa', naar: o.bron, centen: o.centen,
    ...
    if (o.tariefCenten > 0) {
      const t = await boekAsync({ van: 'rtg:reserve', naar: o.bron, centen: o.tariefCenten,
