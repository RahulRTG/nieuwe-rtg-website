...
module.exports = ({ appbieb, reisbieb, rtfbieb, schoolbieb, beroepenbieb, anthropic, schoon }) => {
  ...
  async function adviseer(vraag, { wereld = 'mall', groep = 'volw' } = {}) {
    ...
    const rauw = wereld === 'rtf' ? zoekRtf(q, groep) : zoekMall(q);
    ...
    if (!aanraders.length) {
      return { status: 200, antwoord: wereld === 'rtf'
    ...
    if (anthropic) {
      try {
        const r = await anthropic.messages.create({ model: 'claude-opus-4-8', max_tokens: 400,
          system: 'Je bent de warme bibliothecaris van de RTG Bibliotheek. Schrijf in het Nederlands een kort, persoonlijk advies (max 120 woorden) bij de gevonden apps. ' +
            'Noem ALLEEN apps uit de meegegeven lijst; verzin niets. Beloof nooit toegang of prijzen.' +
    ...
    return { status: 200, demo: true, aanraders,
      antwoord: (wereld === 'rtf' ? 'Goede vraag! ' : 'Mooie vraag. ') + 'Ik zou beginnen met "' + eerste.naam + '" uit de ' + eerste.bieb +
        (aanraders.length > 1 ? ', en daarnaast vindt u hieronder nog ' + (aanraders.length - 1) + ' aanraders die erbij passen.' : '.') +
        (wereld === 'rtf' ? ' Alles is gratis; installeer wat je helpt en haal weg wat je niet gebruikt.' : ' Installeren is bij de pas inbegrepen.') };
  }
