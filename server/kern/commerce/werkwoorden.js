/* ============================================================================
   WERKWOORDEN: VERKLAREN, AFTREKKEN, UITLEGGEN.

   De acht vermogens zelf en hun afhankelijkheden staan in ./werkwoordlijst.js --
   ...
   ander. `verklaar` telt op wat een werkwoord VEREIST; `zonder` haalt weg wat op
   een weggevallen werkwoord LEUNDE. Wie denkt met een van de twee toe te kunnen,
...
const IDS = WERKWOORDEN.map(v => v.id);
const OP_ID = new Map(WERKWOORDEN.map(v => [v.id, v]));
...
function verklaar(lijst) {
  ...
  for (const id of gevraagd) {
    if (!isWerkwoord(id)) {
      geweigerd.push({ werkwoord: id, reden: NIET_GEBOUWD[id] || 'Dit werkwoord bestaat niet in kern/commerce/werkwoordlijst.js.' });
      continue;
    }
    heeft.add(id);
  }
  ...
  while (veranderd) {
    ...
    for (const id of [...heeft]) {
      for (const nodig of werkwoord(id).vereist) {
        ...
        heeft.add(nodig); afgeleid.push({ werkwoord: nodig, door: id });
        veranderd = true;
      }
    }
  }
  /* `toon` staat er altijd bij, en dat is geen vrijgevigheid: een koopbaar dat
     ...
     het enige werkwoord dat de meting bijna overal vond (80 van 100). */
  if (!heeft.has('toon')) { heeft.add('toon'); afgeleid.push({ werkwoord: 'toon', door: 'koopbaar' }); }
...
/* DE ANDERE KANT OP, EN HIJ IS NET ZO NODIG. `verklaar` voegt toe wat een
   werkwoord VEREIST; dit haalt weg wat op een weggevallen werkwoord LEUNDE. Zonder
...
function zonder(lijst, wegLijst) {
  const heeft = new Set((Array.isArray(lijst) ? lijst : []).filter(isWerkwoord));
  const weg = [];
  const haal = (id, door) => {
    if (!heeft.has(id)) return;
    heeft.delete(id); weg.push({ werkwoord: id, door });
    for (const v of WERKWOORDEN) if (v.vereist.includes(id)) haal(v.id, id);
  };
  for (const id of (Array.isArray(wegLijst) ? wegLijst : [])) haal(String(id || ''), 'aanbieder');
  return { heeft: IDS.filter(id => heeft.has(id)), weg };
}

const kan = (koopbaar, id) => !!(koopbaar && Array.isArray(koopbaar.werkwoorden) && koopbaar.werkwoorden.includes(id));
...
/* Waarom een handeling NIET kan, in de taal van de koper. De afrekening en de
   ...
   dezelfde weigering bestaan. Geen zin verzinnen voor een onbekend werkwoord: dan
...
module.exports = { WERKWOORDEN, IDS, NIET_GEBOUWD, werkwoord, isWerkwoord, verklaar, zonder, kan, waaromNiet };
