...
module.exports = ({ kern }) => {
  ...
  /* ELKE RIJ KRIJGT ZIJN DOEL EN ZIJN TERMIJN UIT HET REGISTER, en niet uit de
     ...
     meesturen, dan stond dezelfde belofte op negen plekken (LAT.md regel 4).

     EN `tot: null` BESTAAT HIER NIET MEER. Vijf lagen gaven een kale null, en
     die betekende twee dingen die op een scherm identiek lezen: "loopt door tot
     u hem stopt" en "deze laag houdt geen einddatum bij". De termijn is nu een
     STAND met een uitleg; zie de kop van ./consent-register.js. */
  const perLaag = Object.fromEntries(LAGEN.map(l => [l.id, l]));
  function verrijk(rij) {
