...
const { CONTRACTEN, staat, fout } = require('./document-contracten');

module.exports = function maakDocumentCapability({ bewerkCollectie, store, leesBytes, nu }) {
  return async function documentActie(key, input) {
    const b = input || {};
    ...
    try {
      return await bewerkCollectie('bestanden', alle => {
        ...
        const afdruk = createHash('sha256').update(JSON.stringify([
          b.capability, b.contractVersion, b.id, b.expectedVersion
        ])).digest('hex');
        ...
        if (b.capability === 'document.restore' && [it, ...(it.versies || [])].some(v => !leesBytes(v.ref)))
