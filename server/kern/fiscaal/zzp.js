...
function zzpBerekening(land, winstIn, opties) {
  ...
  if (landCode === 'NL') {
    ...
    const mkb = rondEuro(rest * Z.mkbVrijstelling);
    belastbaar = rondEuro(rest - mkb);
    ...
    belasting = Math.max(0, rondEuro(ib - korting));
    out.posten.push({ label: 'Inkomstenbelasting (schijven)', bedrag: rondEuro(ib) });
    out.posten.push({ label: 'Heffingskortingen (indicatie)', bedrag: -rondEuro(korting) });
