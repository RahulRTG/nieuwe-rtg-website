/* Hospitality Guest OS (deelmodule): DE VERDELING -- wie betaalt welk deel.

   ONDERSCHEID DAT ALLES DRAAGT. Splitsen door de bediening (horeca/schuif.js)
   is een rekening in TWEE REKENINGEN knippen: de tafel gaat uit elkaar. Wat een
   gast doet is iets anders -- de rekening blijft een rekening, en er wordt
   afgesproken wie welk DEEL betaalt. Dat zijn twee handelingen met bijna
   dezelfde naam, en ze door elkaar halen levert een rekening op die twee keer
   bestaat. Daarom ligt de verdeling hier OP de rekening en knipt hij niets.

   DE SOM IS HEILIG, EN OP DEZELFDE MANIER ALS BIJ SCHUIF.JS. De delen tellen
   exact op tot wat er te betalen is; een rest van een paar cent gaat naar de
   EERSTE delen. 10,00 door drie is 3,34 + 3,33 + 3,33. Wie per deel afrondt
   komt op 9,99 of 10,02 uit, en dat verschil komt bij duizend tafels per week
...
module.exports = ({ save, horeca, orderlaag }) => {
  ...
  /* ---------- de verdeling ---------- */
  function verdeel(zaakcode, rek, { wijze, delen, nr }) {
    ...
    /* DE CONTROLE. Dit is geen extra veiligheid maar de kern: een verdeling die
       ...
       plaats van stilzwijgend rechtgetrokken, want dan zou de gast betalen wat
    ...
    rek.verdeling = { wijze: w, teBetalen, delen: uit, at: nu() };
    audit(rek, { actor: 'gast', bron: 'gast', wat: 'verdeling', naar: w + ' over ' + uit.length });
    zetReis(rek, 'afrekenen');
    save();
    return { ok: true, verdeling: rek.verdeling };
  }

  return { WIJZEN, knip, verdeel };
};
