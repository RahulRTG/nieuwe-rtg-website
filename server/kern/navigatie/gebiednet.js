/* DE ROUTEMOTOR VAN EEN GEBIED -- snap, A*, plekken en de kaart eromheen, op
   het pakket van EEN gebied. De grote data staat in RTG_DATA_DIR; de server
   leest alleen lokale SQLite-indexen. De enige externe stap is het verversbare
   bouwscript, nooit een routeverzoek.

   DIT BESTAND HEETTE nederland.js, EN DAT WAS DE TWEEDE WAARHEID ZELF. Drie
   dingen hielden een generieke motor aan een land vast: de graafmap heette
   letterlijk `nederland-graaf`, het vak stond als constante in de code, en
   `land: 'Nederland'` ging mee in elk antwoord. Alle drie komen nu van buiten;
   ./nederland.js geeft ze mee en is EEN geval van deze motor.

   WELK gebied onder een punt ligt beslist ./gebiedkeuze.js -- een rechthoek is
   geen grens, en die nuance hoort niet in een motor. Deze krijgt een gebied
   mee en gelooft het. */
'use strict';
...
/* De prioriteitshoop van de zoeker staat apart: het is een gegevensstructuur
   en geen kaartkennis. Zie ./hoop.js. */
const { Hoop } = require('./hoop');

/* Alles komt van de aanroeper: waar het pakket ligt, welk vak het beslaat, hoe
   het heet en uit welk net het is gebouwd. Ontbreekt het pakket, dan `null` --
   een halve motor levert verderop een stille undefined op. */
function maakGebiedNet({ bestand, graafMap, gebied, haversine }) {
  if (!bestand || !graafMap) return null;
  if (!fs.existsSync(bestand)) return null;
  if (!fs.existsSync(path.join(graafMap, 'graaf.json'))) return null;
  const vak = (gebied && gebied.vak) || null;
  const naam = (gebied && gebied.naam) || '';
  const netwerk = (gebied && gebied.netwerk) || '';
  /* Zonder vak zegt `binnen` FALSE en niet true: een motor die overal "ja"
     zegt, biedt zich op elk punt op aarde als de juiste kaart aan. */
  const binnen = p => !!vak && !!p && Number(p.lat) >= vak.lat0 && Number(p.lat) <= vak.lat1
    && Number(p.lng) >= vak.lng0 && Number(p.lng) <= vak.lng1;
  ...
  function kaart(hier, plekken) {
    const dy = 0.045, dx = dy / Math.max(0.45, Math.cos(hier.lat * Math.PI / 180));
    return { status: 200, netwerk: netwerk || meta.netwerk || '', ref: { lat: hier.lat, lng: hier.lng },
      ...
      dekking: { land: naam, wegvakken: Number(meta.wegvakken || 0), bron: meta.bron, licentie: meta.licentie, gebouwdAt: meta.gebouwd_at } };
  }

  return { snap, zoek, stappenVan, zoekPlekken, kaart, binnen,
    info: { ...meta, ...graaf, land: naam, code: (gebied && gebied.code) || '', actief: true, bestand } };
}

module.exports = { maakGebiedNet };
