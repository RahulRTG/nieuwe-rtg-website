...
module.exports = (ctx) => {
  ...
  const eigen = require('../eigencollectie')({ db, domein: 'kern/overheid/onderneming', bezit: { rijkKvk: 'lijst', rijkUitkeringen: 'lijst' } });
  ...
  function kvkInschrijven(houder, data) {
    ...
    const bestaand = eigen.bak('rijkKvk').find(k =>
    ...
    eigen.bak('rijkKvk').unshift(k);
    eigen.zetBak('rijkKvk', eigen.bak('rijkKvk').slice(0, 60000));
  ...
  function kvkMijn(houder) {
    seed();
    const list = eigen.bak('rijkKvk').filter(k => (houder.key && k.key === houder.key) || (houder.supplierCode && k.supplierCode === houder.supplierCode));
    return { ok: true, inschrijvingen: list.slice(0, 20).map(publiekeKvk) };
  }
  function kvkVoorSupplier(code) { return eigen.bak('rijkKvk').find(k => k.supplierCode === code) || null; }
  ...
  function kvkLijst() {
    seed();
    return { ok: true, inschrijvingen: eigen.bak('rijkKvk').slice(0, 300).map(k => ({ ...publiekeKvk(k), houder: k.houder, viaOnderneming: !!k.supplierCode })) };
  }
  ...
  function uitkeringAanvraag(sess, codenaam, data) {
    ...
    if (eigen.bak('rijkUitkeringen').some(u => u.key === sess.key && u.soort === soort && ['aangevraagd', 'in behandeling', 'toegekend'].includes(u.status)))
    ...
    eigen.bak('rijkUitkeringen').unshift(u);
    eigen.zetBak('rijkUitkeringen', eigen.bak('rijkUitkeringen').slice(0, 40000));
  ...
  function mijnUitkeringen(key) { seed(); return { ok: true, uitkeringen: eigen.bak('rijkUitkeringen').filter(u => u.key === key).slice(0, 30).map(publiekeUitkering) }; }
  function uitkeringenLijst(filter) {
    seed(); filter = filter || {};
    let list = eigen.bak('rijkUitkeringen');
  ...
  function uitkeringBeslis(actor, r, data) {
    data = data || {};
    const u = eigen.bak('rijkUitkeringen').find(x => x.ref === String(r || ''));
