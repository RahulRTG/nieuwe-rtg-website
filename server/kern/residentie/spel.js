...
module.exports = (ctx) => {
  const { R, kamer, kamerVan, sein, sseToCustomer, save } = ctx;
  ...
  function daag(key, body) {
    ...
    potjes()[id] = { spel: body.spel, status: 'wacht', at: Date.now(),
      spelers: [{ key, codenaam: ik.codenaam, punten: [] },
        { key: tegenKey, codenaam: spelerIn(id, tegenKey).codenaam, punten: [] }], beurt: 0 };
    save();
    try { sseToCustomer(tegenKey, 'residentie', { kind: 'spel-uitnodiging', kamer: id, spel: body.spel, naam: S2.naam, van: ik.codenaam }); } catch (e) {}
    return { status: 200, ok: true, wacht: true };
  }
  ...
  const staTe = (p, id) => ({ spel: p.spel, naam: SPELLEN[p.spel].naam, eenheid: SPELLEN[p.spel].eenheid,
    ...
    spelers: p.spelers.map(s => ({ codenaam: s.codenaam, punten: s.punten })),
    aanZet: p.status === 'bezig' ? p.spelers[p.beurt % 2].codenaam : null });
  ...
  function spelZet(key, body) {
    ...
    let uitslag = null;
    if (klaar) {
      const som = s => Math.round(s.punten.reduce((a, b) => a + b, 0) * 10) / 10;
      const [a, b] = p.spelers.map(som);
      uitslag = { stand: [a, b], winnaar: a === b ? null : (S2.laag ? (a < b ? 0 : 1) : (a > b ? 0 : 1)) };
      delete potjes()[id];
    }
    save();
  ...
  /* het vragenspel: aan tafel in het restaurant of in de eigen suite */
  function vraag(key) {
    const id = kamerVan(key);
    if (!id) return { status: 409, error: 'U bent nog geen kamer binnen.' };
    ...
    const tekst = VRAGEN[Math.floor(Math.random() * VRAGEN.length)];
    sein(id, 'vraag', { tekst }, key);
    return { status: 200, ok: true, tekst };
  }
