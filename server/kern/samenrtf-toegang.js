/* Credentiallevensloop van een FoundationOS Samen-kamer. De gezins- en
   vriendschapscontrole blijft een tweede poort in samenrtf.js; deze laag
   bezit uitsluitend het eenmalige deelgeheim en zijn levensloop. */
'use strict';

const DOEL = 'foundationos-samen-kamer';
const SCOPE = ['rtf.samen.join'];
const GELDIG_MS = 6 * 3600000;
const MAX_GEBRUIK = 11;

module.exports = ({ crypto, nu = () => new Date().toISOString() }) => {
  const bearer = require('./bearercode')({ crypto,
    namespace: 'foundationos-samen-kamer', nu });
    ...
    for (let poging = 0; poging < 8; poging++) {
      ...
      const dubbel = rijen.some(x => bearer.zelfdeHash(
        ...
        historie(x).some(h => bearer.zelfdeHash(
          h && h.code_hash, gemaakt.toegang.code_hash)));
  ...
  const publiek = k => {
    const t = k && k.toegang;
    return t ? Object.assign(bearer.publiek(t), {
      stand: reden(k) || 'actief'
    }) : null;
  };
  ...
  function migreerLegacy(kamers) {
    for (const [sleutel, k] of Object.entries(kamers || {})) {
      ...
      const id = /^rsk[a-f0-9]{32}$/i.test(String(k.id || ''))
        ? k.id : 'rsk' + crypto.randomBytes(16).toString('hex');
      k.id = id;
      k.toegang = {
        code_hash: bearer.hash(raw), issuer: k.gastheer || 'legacy',
        doel: DOEL, scope: [...SCOPE],
        onderwerp: { soort: 'rtf-samen-kamer', id }, issued_at: issued,
        expires_at: new Date(Date.parse(issued) + GELDIG_MS).toISOString(),
        max_gebruik: MAX_GEBRUIK,
        gebruik: Math.max(0, (k.leden || []).length - 1),
        laatst_gebruikt_at: null, ingetrokken_at: issued,
        ingetrokken_door: 'legacy-migratie',
        intrekreden: 'legacy code vereist rotatie', rotatie: 1
      };
      k.toegang_historie = [];
  ...
  return { nieuw, zoek, reden, gebruik, intrekken, roteer, publiek,
    migreerLegacy };
};
