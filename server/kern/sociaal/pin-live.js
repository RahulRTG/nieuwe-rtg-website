/* Sociaal (deelmodule): DE LEVENDE CONTACTCODE -- de QR die verloopt.
   ...
   past een code die een halve minuut leeft:
   ...
   - hij VERLOOPT (standaard 60 seconden) en wordt ondertekend door
   ...
   juiste kant van die keuze: ze leven een minuut, ze horen een herstart niet te
   overleven, en er hoort niets van in een back-up te belanden. Zelfde redenering
   als bij het slot in server/pinslot.js. */
module.exports = (ctx) => {
const { crypto, sociaalRate, socialVerbind, pinKijk, dyncodeGeef } = ctx;

const TTL_MS = 60 * 1000;
const UUR = 60 * 60 * 1000;
const MAX_OPEN = 20000;          // begrensde geheugengroei; ze verlopen toch binnen een minuut
...
function liveMaak(handle) {
  ...
  open.set(verwijzing, { handle, vervalt: Date.now() + TTL_MS });
  const c = d.maak({ soort: 'contact', code: verwijzing, ttlMs: TTL_MS });
  return { status: 200, token: c.token, exp: c.exp, ttlMs: c.ttlMs };
}
...
function losOp(token) {
  ...
  if (!v || v.vervalt < Date.now()) { open.delete(r.code); return null; }
...
async function liveVerbind(mij, token) {
  const gevonden = liveKijk(mij, token);
  ...
  return { ...r, codename: gevonden.codename };
}
