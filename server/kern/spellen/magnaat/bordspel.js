/* Magnaat, vorm 'bord': het bordspel met veertig velden, dobbelstenen, huizen,
   kanskaarten en de cel. Verbatim afgesplitst uit ./index.js toen de economie
   ernaast kwam te staan; er is geen regel aan veranderd.

   WAAROM HET BLIJFT. De economie (./economie.js) is een ander spel geworden:
   een stad, bedrijven, personeel, een klok die doorloopt. Dat is precies wat er
   gevraagd was, en het is GEEN vervanging van dit bord -- dit is de enige
   Magnaat die met zes mensen binnen een uur aan tafel te spelen is, en die
   weggooien voor iets groters zou betekenen dat een werkend spel verdwijnt
   omdat er een ambitieuzer spel naast kwam.

   De keuze tussen de twee is een VARIANT (`vorm: 'bord'` of `'economie'`), en
   dat is bijna gratis sinds de variantlaag er is. Zie ../variant.js.

   Het bord zelf (de velden, de kaarten en de huurfactor) is pure data en woont
   in ./bord.js. */
...
module.exports = (ctx) => {
  ...
  function magVolgende(potje) {
    ...
    /* `st.dobbel` wordt hier BEWUST niet leeggemaakt, en dat is een reparatie.
       Hij stond hier wel, en de weergave wordt NA deze aanroep opgebouwd -- dus
       wie op een veld landde dat zijn beurt beeindigde (belasting, een
       kanskaart, andermans straat) zag nooit wat hij had gegooid. Bij een veld
       dat te koop staat ging het wel goed, en dat is de meerderheid; daardoor
       viel het niet op. Aan tafel blijven de stenen liggen tot de volgende
       speler gooit, en `magnaatZet` overschrijft ze bij elke worp. */
    st.mag = 'gooi'; st.koopVeld = null; st.dubbels = 0;
  ...
  const zicht = {
    /* Alles aan tafel is openbaar: posities, geld, bezit en huizen staan bij dit
       spel per definitie open. De weergave leest `mij` niet, dus een kijker en
       een gedeeld scherm zien hetzelfde als een speler. */
  ...
  return { magnaatInit, magnaatZet, zicht, statisch: () => ({ velden: M_VELDEN }), M_VELDEN };
};
