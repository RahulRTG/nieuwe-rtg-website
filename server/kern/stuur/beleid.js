...
function beleidVoor(pad, wereld) {
  ...
  /* Een bodem op `assist` betekent: nooit zonder menselijke bevestiging. In de
     ...
     van de bodempaden staat op een DIRECT-lijst -- en dat is het punt: het was
     tot nu toe waar bij toeval, en het is nu waar bij constructie. Wie morgen
     /api/bank/sepa op de DIRECT-lijst zet, krijgt hier een voorstel terug in
     plaats van een stille uitvoering. */
  const niveau = bodem && bodem.minimum === 'assist' && opDeLijst === 'direct' ? 'voorstel' : opDeLijst;
