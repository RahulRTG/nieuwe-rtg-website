...
module.exports = ({ db, save, crypto, schoon, findSupplier, notify, notifySupplier, sseToSupplier, sseToCustomer }) => {
  ...
  function checkIn(supplier, vid, actorName) {
    ...
    save();
    notify(v.customerKey, { icon: '🗝️', title: supplier.name, body: 'Welkom. Uw kamer is ' + v.roomName + '; alles wat u bestelt kan op de kamer.', scope: 'orders' });
    sseToCustomer(v.customerKey, 'sync', { scope: 'verblijf' });
    sseToSupplier(supplier.code, 'sync', { scope: 'receptie' });
    return { ok: true, verblijf: v };
  }
  ...
  return { verblijfBoek: boek, mijnVerblijven, verblijfAnnuleer: annuleer, verblijfBeslis: beslis, verblijfCheckin: checkIn, verblijfCheckout: checkOut, verblijfNoShow: noShow, receptie };
};
