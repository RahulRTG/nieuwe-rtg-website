...
function maakWaarde({ db, save, crypto, nu = () => Date.now() }) {
  ...
  function registreer({ rek, klasse, uitgever, beleid, vervaltOp }) {
    ...
    posities()[rek] = { klasse, uitgever: uitgever || null, beleid: beleid || {}, vervaltOp: verval, sinds: nu() };
    save();
  ...
  function overzicht(rek, saldo) {
    ...
    return { rek, waardepositie: true, klasse: p.klasse, klasseNaam: p.spec.naam,
      ...
      plafondCenten: p.spec.plafondCenten, ruimte: Number.isFinite(p.spec.plafondCenten) ? Math.max(0, ruimte(rek, s)) : null,
      uitbetaalbaar: p.spec.uitbetaalbaar, overdraagbaar: p.spec.overdraagbaar,
      vervaltOp: p.vervaltOp, beleid: p.beleid, grond: p.spec.grond,
      reserveringen: reserve.open(rek).map(r => ({ id: r.id, centen: r.centen, doel: r.doel, tot: r.tot })) };
  }
