...
module.exports = (ctx) => {
  ...
  async function haalNieuwer(dataNu, opSessieWijziging) {
    ...
    for (const r of rows) {
      ...
      if (lokaalOpen) {
        const basis = JSON.parse(baseJson), hun = JSON.parse(hunJson);
        const bijzonder = voegBijzonderSamen(r.key, basis, dataNu[r.key], hun);
        dataNu[r.key] = bijzonder === null ? merge3(basis, dataNu[r.key], hun) : bijzonder;
        /* Een speciale delta-merge heeft de lokale wijziging op de actuele
           databasebasis herbaseerd. Bewaar die basis, anders telt de latere
           achtergrondcommit dezelfde remote delta nogmaals mee. */
        if (bijzonder !== null) laatsteJson.set(r.key, hunJson);
      } else {
