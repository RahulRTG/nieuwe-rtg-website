...
module.exports = (kern) => {
  ...
  app.post('/api/bank/krediet/aflossing', auth, async (req, res) => { if (gate(req, res)) return; stuur(res, await bank.bankKredietAflossing({ id: String(req.body.id || ''), centen: req.body.centen, codenaam: cn(req) })); });
