...
module.exports = ({ kern, zaakVan, horeca, bezorglaag, beleid, schoon }) => {
  const { app, auth } = kern;
  const etenCapaciteit = require('../../kern/eten/capaciteit');
  app.post('/api/gast/bezorg/kaart', auth, (req, res) => {
    ...
    const capaciteit = etenCapaciteit.bereken(horeca.H(s.code));
    ...
    res.json({ ok: true, zaak: { code: s.code, naam: s.name, stad: s.city || null,
      ...
      open: !(s.settings && s.settings.ordersOpen === false) && capaciteit.open,
      ...
      bezorging: { beschikbaar: zones.length > 0, open: zones.length > 0 && bezorg.open !== false && capaciteit.open,
        minutenVanaf: (minimum('minuten') || 0) + capaciteit.extraMinuten || null,
        afhalenPromoten:capaciteit.afhalenPromoten,
        capaciteitsstand:capaciteit.open ? capaciteit.stand : 'gepauzeerd',
        kostenVanafCenten: minimum('kostenCenten') ||
