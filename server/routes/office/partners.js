...
module.exports = (octx) => {
  ...
  /* DE VERENIGING VAN TWEE KANTEN, en de route onderaan is de reden dat dat
     moest. Deze tak bracht tien nieuwe kantooradressen mee (ondernemersregie,
     rechtsvormwacht, scholen, trust) maar had /api/office/partner/decide NIET --
     en juist daar zit het werk van deze week in: de pas-eis voor een
     bedrijfscode, het abonnement van de zaak, en `rate: 0` in plaats van de 12%
     die het huis allang niet meer belooft. De takkant zonder meer nemen had die
     route laten verdwijnen: het bestand blijft staan, het adres geeft 404, en
     geen enkele meter ziet het. Dat is deze ronde al eens gebeurd en het kostte
     twee uur suite om te vinden. */
  const { accounts, app, appUrl, boardroomAuth, boardroomWie, db, ensureSupplierDefaults,
          findSupplier, forgetSession, logActivity, mail, makeSupplierCode, officeAuth,
          ondernemingBijdrageZet, ondernemingProvisioningZet, ondernemingRegie, rechtsvormwacht,
          save, schoon, sessions, sseClients, sseSend, sseToOffice, sseToSupplier } = kern;
