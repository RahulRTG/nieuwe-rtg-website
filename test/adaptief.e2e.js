/* Adaptief RTG tegen een ECHTE server: de deur en de montage.

   test/adaptief.test.js bewijst het gedrag van de laag. Dit bestand bewijst wat
...
test('de intake loopt over echte routes en kapt zichzelf af', async () => {
  let s = await post('/api/adaptief/intake', {}, token);
  ...
  while (!s.body.klaar) {
    ...
    gehad.push(v.id);
    s = await post('/api/adaptief/antwoord',
...
test('het geheugen toont wat het lid zelf zei, met grond en doel', async () => {
  const g = await post('/api/adaptief/geheugen', {}, token);
...
test('niet-hiervoor-gebruiken werkt, en vergeten haalt het echt weg', async () => {
  const voor = await post('/api/adaptief/geheugen', {}, token);
  const doel = voor.body.neigingen[0];
  assert.ok(doel.doel.includes('tonen'));

  const geweigerd = await post('/api/adaptief/niet-voor', { id: doel.id, doel: 'tonen' }, token);
  assert.equal(geweigerd.status, 200);
  assert.ok(!geweigerd.body.neiging.doel.includes('tonen'));

  const weg = await post('/api/adaptief/vergeet', { id: doel.id }, token);
  assert.equal(weg.status, 200);
  const na = await post('/api/adaptief/geheugen', {}, token);
...
test('een onbekend doel wordt geweigerd en geeft geen lege lijst', async () => {
  ...
  const r = await post('/api/adaptief/niet-voor', { id, doel: 'adverteren' }, token);
...
test('zonder inlog komt er niets uit deze laag', async () => {
  for (const pad of ['/api/adaptief/intake', '/api/adaptief/geheugen']) {
...
test('een tweede lid ziet niets van het eerste', async () => {
  ...
  const g = await post('/api/adaptief/geheugen', {}, ander.body.token);
  assert.equal(g.status, 200);
...
test('overslaan kan, en daarna komt er geen vraag meer', async () => {
  ...
  assert.equal((await post('/api/adaptief/intake', {}, tk)).body.klaar, false);
  assert.equal((await post('/api/adaptief/overslaan', {}, tk)).status, 200);
  assert.equal((await post('/api/adaptief/intake', {}, tk)).body.klaar, true);
  /* En opnieuw beginnen brengt de vragen terug -- overslaan is geen eenrichting. */
  assert.equal((await post('/api/adaptief/opnieuw', {}, tk)).body.klaar, false);
});
