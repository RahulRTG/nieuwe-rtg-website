...
const maak = (wie, bedrag, extra) => api('/uitgave/maak', Object.assign({ omschrijving: 'Laptops voor het team',
  begunstigde: 'Leverancier BV', bedrag, factuur: 'F-2026-17' }, extra || {}, wie));
const keur = (wie, id, recht) => api('/keur', Object.assign({ soort: 'uitgave', id, recht: recht || 'geld.goedkeuren' }, wie));

test.before(async () => {
  ({ child, base: BASE } = await startServer({ env: { RTG_DATA_DIR: TMP, SMTP_URL: '' } }));
  const w = (await api('/werkruimte/maak', { naam: 'RTG Uitgave', land: 'NL' })).body;
...
test('2-3. de indiener keurt niet goed, een ander wel; een regel telt mee', async () => {
  ...
  assert.deepEqual(ander.body.ontbreekt, []);
  const lijst = (await api('/uitgaven', FIN)).body.uitgaven;
  assert.equal(lijst.find(x => x.id === u.id).stand, 'goedgekeurd');

  // een bedrijfsregel boven 1000 euro eist daarnaast de jurist
  const regel = await api('/regel/zet', { werkruimte: W, beheerToken: B, soort: 'uitgave', boven: 1000, eist: ['recht'] });
  ...
  const stand = (await api('/keuring', Object.assign({ soort: 'uitgave', id: groot.id }, FIN))).body;
...
test('4. een tekengrens versmalt, en weghalen haalt de versmalling weg', async () => {
  ...
  const zet = await api('/lid/tekengrens', { werkruimte: W, beheerToken: B, lidId: LIDID[CFO.lidToken], bedrag: 500 });
  ...
  await api('/lid/tekengrens', { werkruimte: W, beheerToken: B, lidId: LIDID[CFO.lidToken], bedrag: '' });
...
test('5-6. betaald noteren: na goedkeuring, niet door de indiener, met kenmerk', async () => {
  ...
  const teVroeg = await api('/uitgave/betaald', Object.assign({ id: u.id, kenmerk: 'BANK-1' }, DIR));
  assert.equal(teVroeg.status, 409, 'nog niet goedgekeurd');
  assert.equal((await keur(DIR, u.id)).status, 200);
  const zelf = await api('/uitgave/betaald', Object.assign({ id: u.id, kenmerk: 'BANK-1' }, FIN));
  assert.equal(zelf.status, 409, 'de indiener vinkt zijn eigen betaling niet af');
  assert.equal((await api('/uitgave/betaald', Object.assign({ id: u.id }, CFO))).status, 400, 'zonder kenmerk');
  const ok = await api('/uitgave/betaald', Object.assign({ id: u.id, kenmerk: 'BANK-1' }, CFO));
  assert.equal(ok.status, 200, JSON.stringify(ok.body));
  assert.equal(ok.body.uitgave.stand, 'betaald');
  assert.match(ok.body.let, /niets overgemaakt/);
  assert.equal((await api('/uitgave/betaald', Object.assign({ id: u.id, kenmerk: 'BANK-2' }, DIR))).status, 409, 'een keer');
  // er is geen route die het bedrag wijzigt
  const wijzig = await api('/uitgave/zet', Object.assign({ id: u.id, bedrag: 1 }, FIN));
