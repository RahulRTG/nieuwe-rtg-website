...
const { digest, evidenceDigest, verify } = require('../scripts/document-evidence');
test('evidence binding rejects changed contract bytes commit capability version and forged PASS', () => {
  ...
  expected.evidenceDigest = evidenceDigest(b);
  ...
  const replaced = { 'run.tap': Buffer.from(files['run.tap'].toString().replace('owned lifecycle proof', 'forged success')) };
  const rehashed = { ...b, files: [{ name: 'run.tap', sha256: digest(replaced['run.tap']) }], runs: [{ ...b.runs[0], requiredTests: ['forged success'] }] };
  assert.equal(verify(rehashed, expected, replaced).valid, false, 'rewriting both evidence and its self-reported hash cannot replace the trusted digest');
  assert.equal(verify(b, { ...expected, evidenceDigest: undefined }, files).valid, false);
});
