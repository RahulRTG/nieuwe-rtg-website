...
async function tafel(naam, stoelen, regels) {
  const r = (await H('/rekening/open', { kanaal: 'tafel', tafel: naam, gasten: stoelen.length })).body.rekening;
  for (const s of stoelen) await H('/gezelschap/stoel', { rekeningId: r.id, handle: s });
  for (const x of (regels || [])) {
    const regel = (await H('/rekening/regel', { rekeningId: r.id, naam: x.naam, prijs: x.prijs,
      aantal: x.aantal || 1, gang: 1, station: 'koud' })).body.regel;
    if (x.voor) await H('/rekening/regel/stoel', { rekeningId: r.id, regelId: regel.id, nr: x.voor });
  }
...
test('gelijk verdelen verliest geen cent: 10,00 door drie is 3,34 + 3,33 + 3,33', async () => {
  ...
  const v = (await H('/rekening/verdeel', { rekeningId: r.id, wijze: 'gelijk' })).body;
...
test('per product betaalt ieder wat op zijn naam staat, en de tafel deelt de rest', async () => {
  ...
  const v = (await H('/rekening/verdeel', { rekeningId: r.id, wijze: 'product' })).body;
...
test('korting en fooi gaan evenredig mee, niet bij een van de twee hangen', async () => {
  ...
  const k = await H('/korting', { rekeningId: r.id, procent: 10, reden: 'stamgast' });
...
test('een verdeling die niet optelt wordt geweigerd, met het getal erbij', async () => {
  ...
  const mis = await H('/rekening/verdeel', { rekeningId: r.id, wijze: 'persoon',
...
test('percentages moeten op honderd uitkomen', async () => {
  ...
  const mis = await H('/rekening/verdeel', { rekeningId: r.id, wijze: 'percentage',
    delen: [{ nr: 1, procent: 60 }, { nr: 2, procent: 30 }] });
  assert.equal(mis.status, 400);
  assert.match(mis.body.error, /90%/);

  const goed = (await H('/rekening/verdeel', { rekeningId: r.id, wijze: 'percentage',
...
test('een tafel zonder stoelen krijgt een reden die zegt wat er moet gebeuren', async () => {
  ...
  const mis = await H('/rekening/verdeel', { rekeningId: r.id, wijze: 'gelijk' });
  assert.equal(mis.status, 409);
...
test('verdelen is geen splitsen: er komt geen tweede rekening bij', async () => {
  ...
  await H('/rekening/verdeel', { rekeningId: r.id, wijze: 'gelijk' });
  const na = (await H('/rekeningen', { status: 'open' })).body.rekeningen;
...
test('de gastdeur en de zaaldeur geven exact dezelfde verdeling', async () => {
  ...
  const qr = (await H('/gast/qr', { tafel: 'V9' })).body;
  const noor = (await api('/api/gast/aanschuiven', { token: qr.token, naam: 'Noor' })).body;
  assert.ok(noor.sleutel, 'de gast schuift aan');
  const open = (await H('/rekeningen', { status: 'open' })).body.rekeningen.find(x => x.tafel === 'V9');
  await H('/gezelschap/stoel', { rekeningId: open.id, handle: 'Kim' });
  ...
  for (const x of regels) {
    const regel = (await H('/rekening/regel', { rekeningId: open.id, naam: x.naam, prijs: x.prijs,
      aantal: 1, gang: 1, station: 'koud' })).body.regel;
    if (x.voor) await H('/rekening/regel/stoel', { rekeningId: open.id, regelId: regel.id, nr: x.voor });
  }

  for (const wijze of ['gelijk', 'product']) {
    const zaak = (await H('/rekening/verdeel', { rekeningId: open.id, wijze: wijze })).body;
