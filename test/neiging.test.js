/* RTG Neiging -- de regels die niet mogen sneuvelen (NEIGING.md par. 5).

   Alles hier draait zonder server, want ./ladder.js en ./vraag.js zijn puur en
   ./bewaren.js heeft alleen een db-achtig object nodig. De HTTP-kant staat in
   test/neiging.e2e.js: die bewijst de montage en de deur, en deze bewijst het
...
const ladder = require('../server/kern/neiging/ladder');
const vraag = require('../server/kern/neiging/vraag');
const maakBewaren = require('../server/kern/neiging/bewaren');
const maakNeigingLaag = require('../server/kern/neiging');
...
function huis(nu) {
  ...
  return { db, klok, tel: () => bewaard, neiging: maakBewaren(deps), laag: maakNeigingLaag(deps) };
}
