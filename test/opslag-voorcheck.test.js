...
/* ============================================================================
   ...
   met vergeetAlles() als reset; scripts/staat.js leest die belofte na in de
   ...
   ========================================================================== */
test('resetcontract: na vergeetAlles() antwoordt de module als een verse kopie', async () => {
  ...
  try {
    /* TWEE ECHTE KOPIEEN van dezelfde module. `vers` wordt nooit aangeraakt en
       ...
       bij een vergeetAlles() die alleen laatsteCheck leegde (LAT-regel 9). */
    ...
    assert.deepEqual(antwoorden(werk), antwoorden(vers),
      'na vergeetAlles() hoort deze module op elke invoer te antwoorden als een kopie die nooit iets heeft gezien');
    ...
    /* WAT DEZE VERGELIJKING WEL EN NIET DEKT, want dat is nagemeten en niet
       ...
       mutaties bevestigden dat -- een vergeetAlles() die laatsteLengte laat
       staan, en een die laatsteCheck laat staan, bleven allebei groen.

       Die twee worden gedekt door de BRONPOORT: STATE.json noemt vergeetAlles()
    ...
    werk.vergeetAlles(); vers.vergeetAlles();
  } finally { o.op(); }
});
