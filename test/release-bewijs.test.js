...
test('releasebewijs detecteert ook een toegevoegd runtimebestand', () => {
  ...
  for (const map of ['server', 'public/dist', 'scripts', 'motor/src', 'motor/target/release']) fs.mkdirSync(path.join(root, map), { recursive: true });
  for (const rel of ['package.json', 'package-lock.json', 'motor/Cargo.toml', 'motor/Cargo.lock', 'Dockerfile', 'docker-compose.yml', '.env.example', 'SLO.json'])
  ...
  const bestanden = [];
  for (const rel of ['package.json', 'package-lock.json', 'motor/Cargo.toml', 'motor/Cargo.lock', 'Dockerfile', 'docker-compose.yml', '.env.example', 'SLO.json', 'motor/target/release/rtg-motor', 'motor/target/release/rtg-sentinel'].sort()) {
