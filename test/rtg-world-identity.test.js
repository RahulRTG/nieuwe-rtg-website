/* DE HERITAGE-ROUTEKAART IS FAIL-CLOSED.
   ...
   hier tegen de werkelijke HTML-boom gehouden: 276 schermen, 16 doorwijzers,
   ieder exact eenmaal. */
...
test('het manifest dekt 276 echte schermen en 16 redirects precies eenmaal', () => {
  ...
  assert.equal(BESTANDEN.length, 292, 'de appboom hoort 292 HTML-bestanden te bevatten');
  assert.equal(ECHTE_ROUTES.length, 276, 'exact 276 blijvende schermen horen basis.js te laden');
  assert.equal(DOORWIJZERS.length, 16, 'exact 16 oude adressen horen doorwijzers te blijven');
  assert.equal(echtManifest.length, 276, 'het vierwereldenmanifest hoort 276 schermen te bevatten');
