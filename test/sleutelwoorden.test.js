...
test('instellen: precies vier verschillende woorden van minstens drie letters', async () => {
  const sw = opstelling({});
  assert.equal(sw.swInfo('u1').gezet, false, 'nog niets ingesteld');
  assert.equal((await sw.swZet('u1', ['een', 'twee', 'drie'])).status, 400, 'drie woorden is te weinig');
  assert.equal((await sw.swZet('u1', ['een', 'twee', 'drie', 'vier', 'vijf'])).status, 400, 'vijf woorden is te veel');
  assert.equal((await sw.swZet('u1', ['ja', 'twee', 'drie', 'vier'])).status, 400, 'een woord van twee letters mag niet');
  assert.equal((await sw.swZet('u1', ['appel', 'appel', 'drie', 'vier'])).status, 400, 'vier keer hetzelfde niet');
  const ok = await sw.swZet('u1', ['Lavendel', 'kompas', 'orkaan', 'veranda']);
...
test('de roterende uitdaging: twee woorden los in een zin, dan het derde', async () => {
  const woorden = ['lavendel', 'kompas', 'orkaan', 'veranda'];
  const sw = opstelling({ 'lisa@x.nl': { id: 'u1' } });
  assert.equal((await sw.swZet('u1', woorden)).ok, true);
  ...
  const open = await sw.swZeg(start.id, zin);
  ...
  const slot = await sw.swZeg(start.id, woorden[open.posSluit]);
  assert.equal(slot.ok, true, 'het derde woord sluit de inlog af');
  assert.equal(slot.userId, 'u1', 'en levert het echte gebruikers-id');
});

test('verkeerde woorden komen er niet in', async () => {
  const woorden = ['lavendel', 'kompas', 'orkaan', 'veranda'];
  const sw = opstelling({ 'lisa@x.nl': { id: 'u1' } });
  await sw.swZet('u1', woorden);

  const start = sw.swStart('lisa@x.nl');
  // juiste twee openen, maar een fout slotwoord
  await sw.swZeg(start.id, woorden[start.posA] + ' en ' + woorden[start.posB]);
  const mis = await sw.swZeg(start.id, 'tulp');
  ...
  const o2 = await sw.swZeg(s2.id, 'tulp en hyacint');
  assert.equal(o2.echo, null, 'bij een miskleun in de opening geen echo');
  const eind = await sw.swZeg(s2.id, woorden[o2.posSluit]);
  assert.equal(eind.status, 401, 'een verkeerde opening blokkeert de hele inlog');
});

test('een onbekend account krijgt een lokvink-uitdaging (geen enumeratie)', async () => {
  const sw = opstelling({ 'lisa@x.nl': { id: 'u1' } });
  await sw.swZet('u1', ['lavendel', 'kompas', 'orkaan', 'veranda']);
  ...
  const open = await sw.swZeg(start.id, 'wat dan ook en nog iets');
  assert.equal(open.echo, null, 'er valt niets te herkennen');
  const eind = await sw.swZeg(start.id, 'zomaar');
  assert.equal(eind.status, 401, 'de lokvink faalt aan het eind, net als een echte misser');
});

test('vijf misgelopen pogingen zetten het account op slot', async () => {
  const woorden = ['lavendel', 'kompas', 'orkaan', 'veranda'];
  const sw = opstelling({ 'lisa@x.nl': { id: 'u1' } });
  await sw.swZet('u1', woorden);
  ...
  for (let i = 0; i < 5; i++) {
    ...
    await sw.swZeg(s.id, 'fout en verkeerd');
    const r = await sw.swZeg(s.id, 'mis');
  ...
  await sw2.swZet('u1', woorden);
  const g = sw2.swStart('lisa@x.nl');
  const o = await sw2.swZeg(g.id, woorden[g.posA] + ' en ' + woorden[g.posB]);
  assert.equal((await sw2.swZeg(g.id, woorden[o.posSluit])).ok, true, 'zonder slot laat de juiste combinatie door');
});

test('de uitdaging verloopt en kent zijn id daarna niet meer', async () => {
  const sw = opstelling({ 'lisa@x.nl': { id: 'u1' } });
  await sw.swZet('u1', ['lavendel', 'kompas', 'orkaan', 'veranda']);
  const onbekend = await sw.swZeg('bestaat-niet', 'lavendel en kompas');
